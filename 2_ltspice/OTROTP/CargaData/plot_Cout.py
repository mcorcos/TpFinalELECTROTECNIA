...
for i in range(len(my_data2)):
	...
	if not (math.isnan(x1) or math.isnan(y1)):
		x2.append(x1)
		y2.append(y1)
my2 = [element * 1 for element in y2] # Multiplied to see in mV
...
V_s = sp.symbols('I', cls = sp.Function)
#################################################################################
# Calculo diferencial
#################################################################################
# Ecuación diferencial
eq = C * R1 * V_s(t_s).diff(t_s,1) + V_s(t_s) - V0

# Igualo a las condiciones iniciales
init_values = {V_s(0) : VC0}

#Resolvemos la ecuación diferencial
res = sp.dsolve(eq, V_s(t_s), ics = init_values).args
#print(res)
V_symbol = res[1]

V_lambda = sp.lambdify(t_s, V_symbol, modules = 'numpy')

t = np.linspace(0, .020, 10000)
IL = V_lambda(t)
ax_f.plot(t+0.005, IL, 'orange')

t2 = np.linspace(0, .020, 10000)
VL = (V0/R1)*np.exp(-(1/(R1*C))*t2)
ax_c.plot(t+0.005, VL, 'orange')
#################################################################################
# EXTRA
#################################################################################
plt.grid(True)
